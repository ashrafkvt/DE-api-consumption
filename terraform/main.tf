...
resource "snowflake_storage_integration" "s3_integration" {
  ...
  enabled = true
  storage_provider = "S3"
  storage_aws_role_arn = "arn:aws:iam::816207356993:role/SnowflakeAccessRole"
  storage_allowed_locations = [ "*" ]
  type = "EXTERNAL_STAGE"
}
...
resource "snowflake_stage" "s3_stage" {
  ...
  url = "s3://${aws_s3_bucket.data_lake_bucket.id}/"
  file_format = "TYPE = JSON"
}
...
resource "snowflake_grant_privileges_to_account_role" "access_to_tables" {
  ...
  depends_on = [
    snowflake_table.raw_json_movies,
    snowflake_table.raw_json_genres,
    snowflake_table.raw_json_languages
  ]
}
